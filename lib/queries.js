...
setInterval(function() {
  ...
  conf.connection.query('SELECT 1', function(err, rows) {
    if (err) {
	  pmx.notify('Error: connection to Mysql failed');
	  ...
	  conf.connection.connect();
	}
  });

  conf.connection.query('SHOW FULL PROCESSLIST', function(error, results, infos) {
  if (!error)
  ...
  conf.connection.query('SHOW GLOBAL STATUS', function(error, results, infos) {
    if (!error) {
